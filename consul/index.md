...
一般 consul server 只监听 localhost，通过 nginx 加认证的方式来代理访问。
...
Consul使用consensus protocol（共识协议）来提供一致性(consistency)，它基于**Raft**(In search of an Understandable Consensus Algorithm)。
